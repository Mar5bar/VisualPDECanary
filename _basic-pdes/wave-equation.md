...
1. Change its value by clicking <span class='click_sequence'>{{ layout.equations }} → **Parameters and variables** → **Parameters**</span> and editing the value of $D$: try increasing it by a factor of 10 or even 100. 
...
You can play with such an initial condition in this [initialised simulation](/sim/?preset=waveEquationICs), changing $n$ and $m$ in <span class='click_sequence'>{{ layout.equations }} → **Parameters and variables** → **Parameters**</span> and restarting the simulation by pressing {{ layout.erase }} to see how these parameters influence the solution. 
