...
This example is based on this [paper by Dan Hill and collaborators]() which studies symmetric localized solutions of the Swift-Hohenberg equation. The default initial condition picks out a solution with $D_4$ symmetry. If you change the parameter $P$ to the value $2$ and then press , this will replace the initial condition with one that evolves to a hexagonal solution ($D_6$ symmetry).
